...
/**
 * Checks if workspace node or its children matches search query
 */
export function hasWorkspaceSearchMatch(node: BrowserTreeNode, query: string): boolean {
  ...
  if (node.children) {
    return node.children.some((c) => hasWorkspaceSearchMatch(c, query));
  }
  return false;
}
